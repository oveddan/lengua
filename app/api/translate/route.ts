...
export async function POST(request: NextRequest) {
  ...
  // Check if input is a single word (likely a verb to conjugate)
  const isSingleWord = sentence.trim().split(/\s+/).length === 1;

  let analysisPrompt: string;

  if (isSingleWord) {
    // Single word - check if it's a verb and provide conjugations with example sentences
    analysisPrompt = `Analyze this Spanish word for a language learner.
...
If NOT a verb, use this simpler format:
{
  ...
  "words": [
    ...
    // Multiple words - analyze as sentence
    analysisPrompt = `Analyze this Spanish sentence for a language learner.
...
Respond in JSON only:
{
  ...
  const textContent = analysisMessage.content.find((block) => block.type === 'text');
